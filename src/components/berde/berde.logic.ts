import type {
  ...
  BerdeContext,
  BerdeInputs,
} from '@/components/berde/berde.types';

// Priority order: hype > worried > proud > sarcastic > neutral
// First match wins — most emotionally significant state takes over.
...
export function resolveBerdeState(inputs: BerdeInputs): BerdeContext {
  // HYPE — highest priority, celebratory moments
  ...
  if (inputs.monthSpend === 0) {
    return { state: 'hype', quote: pickQuote('hype', 1), triggerReason: 'Month starts at ₱0' };
  }

  // WORRIED — second priority, danger signals
  ...
  if (inputs.lowSpendStreakDays >= 3) {
    return { state: 'proud', quote: pickQuote('proud', 3), triggerReason: `${inputs.lowSpendStreakDays}-day low spend streak` };
  }

  // SARCASTIC — specific bad patterns
