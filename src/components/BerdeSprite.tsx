...
const EYES: Record<BerdeState, Pixel[]> = {
  ...
  helper: [
    ...
    [6,10,'P'],[7,10,'P'],[7,9,'W'],
    ...[15,16,17,18].flatMap(x => [[x,9,'K'],[x,10,'K'],[x,11,'K'],[x,12,'K']] as Pixel[]),
    [16,10,'P'],[17,10,'P'],[17,9,'W'],
  ],
  excited: [
    [4,7,'Y'],[5,7,'Y'],[6,7,'Y'],[7,7,'Y'],[8,7,'Y'],[9,7,'Y'],
    [4,8,'Y'],[9,8,'Y'],[4,9,'Y'],[9,9,'Y'],
    [4,10,'Y'],[5,10,'Y'],[6,10,'Y'],[7,10,'Y'],[8,10,'Y'],[9,10,'Y'],
    [5,8,'K'],[6,8,'K'],[7,8,'K'],[8,8,'K'],
    [5,9,'K'],[6,9,'K'],[7,9,'K'],[8,9,'K'],
    [6,7,'W'],[7,7,'W'],
    [14,7,'Y'],[15,7,'Y'],[16,7,'Y'],[17,7,'Y'],[18,7,'Y'],[19,7,'Y'],
    [14,8,'Y'],[19,8,'Y'],[14,9,'Y'],[19,9,'Y'],
    [14,10,'Y'],[15,10,'Y'],[16,10,'Y'],[17,10,'Y'],[18,10,'Y'],[19,10,'Y'],
    [15,8,'K'],[16,8,'K'],[17,8,'K'],[18,8,'K'],
    [15,9,'K'],[16,9,'K'],[17,9,'K'],[18,9,'K'],
    [16,7,'W'],[17,7,'W'],
  ],
};
...
const MOUTHS: Record<BerdeState, Pixel[]> = {
  ...
  motivational: [
    [9,17,'K'],[10,17,'K'],[11,17,'K'],[12,17,'K'],[13,17,'K'],[14,17,'K'],[15,17,'K'],
    [10,18,'K'],[11,18,'K'],[12,18,'K'],[13,18,'K'],[14,18,'K'],
    [11,18,'R'],[12,18,'R'],[13,18,'R'],
  ],
  celebratory: [
    [8,16,'K'],[9,16,'K'],[10,16,'K'],[11,16,'K'],[12,16,'K'],[13,16,'K'],[14,16,'K'],[15,16,'K'],[16,16,'K'],
    [8,17,'K'],[16,17,'K'],
    [9,17,'W'],[10,17,'W'],[11,17,'W'],[12,17,'W'],[13,17,'W'],[14,17,'W'],[15,17,'W'],
    [9,18,'R'],[10,18,'R'],[11,18,'R'],[12,18,'R'],[13,18,'R'],[14,18,'R'],[15,18,'R'],
  ],
  helper: [
    [10,17,'K'],[11,17,'K'],[12,17,'K'],[13,17,'K'],[14,17,'K'],
    [10,18,'K'],[14,18,'K'],
    [11,18,'R'],[12,18,'R'],[13,18,'R'],
  ],
  excited: [
...
const BROWS: Record<BerdeState, Pixel[]> = {
  ...
  motivational:[[5,6,'g'],[6,6,'g'],[7,7,'g'],[8,7,'g'],[15,7,'g'],[16,7,'g'],[17,6,'g'],[18,6,'g']],
  celebratory:[[5,5,'g'],[6,5,'g'],[7,5,'g'],[8,5,'g'],[15,5,'g'],[16,5,'g'],[17,5,'g'],[18,5,'g']],
  helper:[[5,7,'g'],[6,7,'g'],[7,7,'g'],[8,7,'g'],[15,7,'g'],[16,7,'g'],[17,7,'g'],[18,7,'g']],
  excited:[[5,4,'g'],[6,4,'g'],[7,4,'g'],[8,4,'g'],[15,4,'g'],[16,4,'g'],[17,4,'g'],[18,4,'g']],
};
...
const EXTRAS: Record<BerdeState, Pixel[]> = {
  ...
  sarcastic: [],
  motivational: [[19,15,'X'],[20,14,'X'],[20,15,'X'],[20,16,'X'],[21,15,'X']],
  celebratory: [[1,3,'Y'],[3,1,'Y'],[20,2,'Y'],[22,4,'Y'],[0,9,'Y'],[23,7,'Y'],[2,19,'X'],[21,19,'X']],
  helper: [[18,14,'K'],[19,14,'K'],[20,14,'K'],[18,15,'K'],[20,15,'K'],[18,16,'K'],[19,16,'K'],[20,16,'K'],[17,16,'Y'],[21,16,'Y']],
  excited: [[2,3,'Y'],[4,2,'Y'],[21,3,'Y'],[22,6,'Y'],[1,11,'Y'],[23,10,'Y'],[4,19,'X'],[19,19,'X']],
};
...
export default function BerdeSprite({ size = 64, state, animated = true }: BerdeSprite) {
  ...
  useEffect(() => {
    ...
    const loop = (ts: number) => {
      ...
      // Lightweight state-specific embellishments without changing the core sprite sheet.
      if (animated && (currentState === 'celebratory' || currentState === 'excited')) {
        const confettiColumns = currentState === 'celebratory' ? [2, 6, 10, 14, 18, 22] : [4, 9, 14, 19];
        const confettiStride = currentState === 'celebratory' ? 3 : 4;
        const confettiPalette: PaletteKey[] = ['Y', 'X', 'L'];
        const tick = Math.floor(ts / 120);

        for (let i = 0; i < confettiColumns.length; i++) {
          const y = (tick + i * confettiStride) % 24;
          const color = confettiPalette[(tick + i) % confettiPalette.length];
          pixels.push([confettiColumns[i], y, color]);
        }
      }
